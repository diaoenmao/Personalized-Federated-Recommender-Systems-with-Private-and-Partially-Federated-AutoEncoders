...
class Netflix(Dataset): 
    data_name = 'Netflix'
    filename = 'netflix_archive.zip'  # https://www.kaggle.com/datasets/netflix-inc/netflix-prize-data?resource=download
    ...
    def __init__(self, root, split, data_mode, target_mode, transform=None):
        self.sliced_user_count = 7000
        # self.sliced_item_count = 8000
    ...
    def process_netflix(self, data_i):
        ...
        # print('extend_user_id length: ', len(extend_user_id))

        # extend_user_id = np.array(extend_user_id)
        # extend_user_id = extend_user_id.astype(np.int64)
        # data_i = data_i.drop(index)
        # movie_id = data_i.iloc[:, 0].astype(np.int64)
        # rating = data_i.iloc[:, 1].astype(np.float32)
        # print('rating length: ', len(rating))

        movie_id = data_i.iloc[index, 0].to_numpy()
        ...
        extend_movie_id = []
        for i in range(len(movie_id)):
            cur_movie_id = movie_id[i]
            cur_movie_id_extend = [cur_movie_id] * (numpy_index[i+1] - numpy_index[i] - 1)
            extend_movie_id.extend(cur_movie_id_extend)

        print('extend_movie_id length: ', len(extend_movie_id))

        extended_movie_id = np.array(extend_movie_id).astype(np.int64)
        # extend_user_id = extend_user_id.astype(np.int64)
        data_i = data_i.drop(index)
        user_id = data_i.iloc[:, 0].astype(np.int64)
        rating = data_i.iloc[:, 1].astype(np.float32)
        print('rating length: ', len(rating))
        return user_id, extended_movie_id, rating
    ...
    def make_explicit_data(self):
        ...
        data_i = pd.read_csv(os.path.join(self.raw_folder, 'combined_data_1.txt'), delimiter=',', names=["movie_id", "rating", "timestamp", 'user_id'])
        user_i, item_i, rating_i = self.process_netflix(data_i=data_i)
        ...
        dense_user_mask = count_nonzero_user >= 0
        dense_item_mask = count_nonzero_item >= 0
        ...
        print('Dense length: user, item, rating: ', len(user), len(item), len(rating))

        user_id, user_inv = np.unique(user, return_inverse=True)
        item_id, item_inv = np.unique(item, return_inverse=True)
        M, N = len(user_id), len(item_id)
        print('unique length: user, item: ', M, N)
        ...
        item_id, item_inv = np.unique(item, return_inverse=True)
        M, N = len(user_id), len(item_id)
        print('sliced unique length: user, item, rating: ', M, N, len(rating))
        user_id_map = {user_id[i]: i for i in range(len(user_id))}
        item_id_map = {item_id[i]: i for i in range(len(item_id))}
        user = np.array([user_id_map[i] for i in user_id], dtype=np.int64)[user_inv].reshape(user.shape)
        item = np.array([item_id_map[i] for i in item_id], dtype=np.int64)[item_inv].reshape(item.shape)      
    ...
    def make_implicit_data(self):
        ...
        item = item[sliced_mask]
        rating = rating[sliced_mask]
        print('implicit rating divide', rating, len(rating), np.sum(rating < 3), np.sum(rating < 3.5), np.sum(rating >= 3.5))
        user_id, user_inv = np.unique(user, return_inverse=True)
        item_id, item_inv = np.unique(item, return_inverse=True)
        M, N = len(user_id), len(item_id)
