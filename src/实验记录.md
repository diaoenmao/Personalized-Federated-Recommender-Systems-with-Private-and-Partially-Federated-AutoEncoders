...
2. 测试average all (1M):
   1. 测试local epoch个数的影响, explicit:
      1. **node=100, fraction=0.1, global_epoch=400, local epoch=5，不加global optimizer, rmse=0.9051, epoch=401**
      2. **node=100, fraction=0.1, global_epoch=400, local epoch=10，不加global optimizer, rmse=0.9136, epoch=373**
      3. **node=100, fraction=0.1, global_epoch=800, local epoch=5，不加global optimizer, rmse=0.8933, epoch=752**
      4. **node=100, fraction=0.1, global_epoch=800, local epoch=10，不加global optimizer, rmse=0.9092, epoch=637** 
   ...
   3. 测试local epoch个数的影响, implicit:
      1. **node=100, fraction=0.1, global_epoch=400, local epoch=5，不加global optimizer, MAP=0.8202, epoch=304**
      2. **node=100, fraction=0.1, global_epoch=400, local epoch=10，不加global optimizer, MAP=0.8203, epoch=335**
      2. **node=100, fraction=0.1, global_epoch=800, local epoch=5，不加global optimizer,  MAP=0.8202, epoch=304**
      2. **node=100, fraction=0.1, global_epoch=800, local epoch=10，不加global optimizer, MAP=0.8201, epoch=97**
   ...
   4. 测试Normalization的影响, implicit，固定global_epoch=400，local_epoch=5
      ...
      2. node=100, fraction=0.1, global_epoch=400, local epoch改为5，加batch normalization, 
   ...
   5. 测试1个user1个node, explicit
      1. node=100, fraction改为0.1, global_epoch=800, local epoch改为5，no normalization, MAP=0.8203, epoch=304
      2. node=100, fraction改为0.1, global_epoch=800, local epoch改为5，加batch normalization, MAP=0.8203, epoch=304
      3. node=100, fraction改为0.1, global_epoch=800, local epoch改为5，加layernorm, MAP=0.8203, epoch=304
   6. 测试1个user1个node, implicit
      1. node=100, fraction改为0.1, global_epoch=400, local epoch改为5，no normalization, MAP=0.8203, epoch=304（同2.3.1）
      2. node=100, fraction改为0.1, global_epoch=800, local epoch改为5，加batch normalization, MAP=0.8203, epoch=304
      2. node=100, fraction改为0.1, global_epoch=800, local epoch改为5，加layernorm, MAP=0.8203, epoch=304
3. 测试只average decoder:
   1. 测试local epoch个数的影响, explicit，固定global_epoch为800:
      1. **node=100, fraction=0.1, global_epoch=800,  local epoch=5，不加global optimizer, rmse=0.9849, epoch=187**
      2. **node=100, fraction=0.1, global_epoch=800,  local epoch=10，不加global optimizer, rmse=0.9883, epoch=121**
   ...
   2. 测试Normalization的影响, explicit:
      ...
      2. node=100, fraction=0.1, global_epoch=800, local epoch=5，batchnorm
   ...
   4. 测试Normalization的影响, implicit
      ...
      2. node=100, fraction=0.1, global_epoch=400, local epoch=5，batchnorm
      3. **node=100, fraction=0.1, global_epoch=400, local epoch=5，layernorm,MAP=0.8287, epoch=259**
   5. 测试1个user1个node, explicit
   2. 测试1个user1个node, implicit
4. 总结:
...
5. 修改:
   1. 进行1.3和1.4时收敛太慢 => 修改batch_size为10
   2. 训练太慢 => 整5个colab
   3. 内存不够存储每个user一个模型，如果是all federated可以优化到模型在train时创建, 然后加载global_model的参数然后跑。而如果是只average decoder，该如何处理（需要保存各个encoder的记录), 1M量级都有6000个user, 20M可能就是8w个。（暂时放着）
      1. 去掉local_test_dict
      2. 存本地再读
   4. 处理BN代码 (load_state_dict中strict=False, 不平均running_mean, running_var)
   5. 优化fed.py流程 （减少内存占用）
