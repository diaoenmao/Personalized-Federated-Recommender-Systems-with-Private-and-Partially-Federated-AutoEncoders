...
6. 修改:
   ...
   12. 5/18/2022
       1. 测试, 验证1 user/node
          1. 验证train_privacy_all.py的正确性:
             1. Average all, 100 nodes, rmse = 0.9297
             2. Average all, 300 nodes, rmse = 0.9348
             3. **Average all, 1 user / node, rmse = 0.9682 (这么高吗)**
             4. 总结: 一开始跑100K的1user / node发现rmse很高，用node=100, node=300验证了下，不是代码问题
          2. ML100K:
             1. joint, rmse=0.9333
             2. Average all, 1node, rmse=0.9303
             3. Average decoder, 3 users / node, rmse=0.9069
             4. Average decoder, 1 user / node, rmse=0.9058
             5. Average all, 1 user / node, rmse = 0.9682
             6. 总结: 
                1. 单纯的average all在极端情况下，rmse很高。
                2. ML100K的极端情况效果非常好
          3. ML1M: 有一个system ram peak
             1. Average all, 2000 nodes (3 users / node): rmse=0.9748
             2. Average decoder, 300 nodes: rmse=0.8735
             3. Average decoder, 2000 nodes (3 users / node): rmse=0.8917
             4. 总结: 
                1. 单纯的average all在极端情况下，rmse很高。 
                2. ML1M的average decoder, 3user/node, rmse相比ml100k 1user/node 没降太多, 3 user/node 反而比 1user/node rmse高？
                3. average decoder极端情况比300 nodes rmse高了1.8
                4. Server side
          4. 按种类分
          5. semi-supervise learning
          6. Node gradient方向不同, 看feature
   ...
   1. find dataset
      ...
      **原始样本骨架raw_sample**
      ...
      2. 处理流程:
         ...
         1. 核实数据:
            1. 检查user_id是否为Unique
               1. 是
   
            2. 检查ad_id(adgroup_id)是否为Unique
               1. 是
   
            3. 检查是否有一个用户点击同一广告多次的事件
               1. 是
               2. 只要用户点击了一个广告，即算点击
   
         2. 初步处理:
            1. 将user_profile.csv中没有属性缺失的用户提取出来，存至/rough_process下
            2. 在raw_sample.csv中取出2.1中没有属性缺失的用户的row，存至/rough_process下
            3. 对于ad_feature.csv暂时不用处理missing value
   
         3. 精细处理:
            1. 从rough_process_user_profile.csv中随机抽取指定数量的user_id
            2. 从rough_process_ad_feature.csv中随机抽取指定数量的ad_id
            3. 选择交集拼接成Matrix => 重合项太少 => 6000user, 3000item只有500个数据点
            3. user rank by dense
            3. 单选6000个user =》有136508个数据，74723个item
            3. item rank by dense
            7. 对比:
               1. ML1M: 
                  1. user: 6040
                  2. item: 3706
...
3. **batchnorm，group norm(layernorm) 区别**
...
4. paper实验:
   ...
   4. 测试fedavg
      ...
      2. Baseline:
         1. 数据集Joint
      3. 在加user attribute的情况下:
         1. 在explicit情况下:
            1. 测试不同node数目情况下， 全average的效果
            2. 测试不同node数目情况下， 只average decoder的效果
            3. 在只average decoder的情况下，获取空间占比compress与不compress
         2. 在implicit情况下:
            1. 测试不同node数目情况下， 全average的效果
            2. 测试不同node数目情况下， 只average decoder的效果
      4. 在不加user attribute的情况下:
         1. 在explicit情况下:
            1. 测试不同node数目情况下， 全average的效果
            2. 测试不同node数目情况下， 只average decoder的效果
            3. 在只average decoder的情况下，获取空间占比compress与不compress
         2. 在implicit情况下:
            1. 测试不同node数目情况下， 全average的效果
            2. 测试不同node数目情况下， 只average decoder的效果
      5. 获取在使用Global model minimum与local model minimum情况下准确度的不同 （暂时不测， 需要验证）
      6. movielens
         1. explicit
            1. ML100K
               1. 100nodes
...
5. 4/28/2022
   ...
   1. 每个数据集1node情况下加入batchnorm和no norm的8个实验

6. 5/2/2022

   1. 加入global model在Local node上的表现和只average decoder的对比

7. 8/27/2022

   1. ML1M:
      1. ML1M, joint, rmse=0.8606
      2. ML1M, node=100, decoder, noinfo, rmse=0.8613
      3. ML1M, node=100, decoder, info, rmse=0.8615
      4. ML1M, node=100, all, noinfo, rmse=0.8628
   2. Douban:
      1. 如果lr太高，一下rmse就碰到最低点，然后rmse反弹
      2. 换成sgd lr=0.01, fedavg all node=100能碰到0.87而后不反弹, 但是fedavg de node=100碰到0.89就直接反弹。同时lr=0.01, node数量一多，**rmse下降太慢**并且**导致无法固定一个global epoch画图**。如果global epoch设置多了，那么那些回弹的就要显示出来，如果设置少了，node数量多的情况下又无法收敛
...
8. Scheduler
   ...
   2. Scheduler
      ...
      7. 基础知识
         ...
         1. ![image-20220828213959061](/Users/qile/Library/Application Support/typora-user-images/image-20220828213959061.png)
         2. ![image-20220828214117789](/Users/qile/Library/Application Support/typora-user-images/image-20220828214117789.png)
         3. ![image-20220828214421725](/Users/qile/Library/Application Support/typora-user-images/image-20220828214421725.png)
         4. Content-based filtering
            1. 根据用户的Profile, item descriptions和**过往的喜爱记录**推荐
         5. collaborative filtering
            1. 用各方面的历史记录和相似用户的喜好推荐，（不用item content information）
            2. 解决推荐系统中建模用户兴趣 (user interest / user preference) 的问题
            3. 兴趣分为长期兴趣/静态兴趣(long-term interest)，短期兴趣/动态兴趣(short-term interest)
            4. 实际上是推荐系统中，利用用户行为，即用户对商品的行为（如购买，收藏，点赞，评分），建模用户的兴趣，并依据此兴趣为用户作推荐。朴素的CF，实际上利用用户行为的中购买行为来建模用户的兴趣
            5. ![img](https://pic1.zhimg.com/80/v2-51aa79866d227196d169b98dac758fe8_720w.jpg)
            6. 推荐算法分三大类：CF，content-based和混合。content-based通常会利用user和item的本身的内容信息，而CF则是behavior信息。混合就是三者或两者结合。
            7. CF算相似度，而相似度必须通过协同其他用户计算
            8. **模型的训练速度也要考虑，计算和存储成本，因此，并行或分布式CF也成为了枝桠**
            9. 在RSs中，user interact很小一部分数据，导致了data sparsity问题，data sparsity主要有2个问题 （cold-start问题，因为一开始没法协同）:
               1. the lack of data to effectively model users’ preferences
               2. the lack of data to effectively model item characteristics
               3. CF好像都based on matrix factorization
      ...
      9. Proposed Titles:
         ...
         Qi: Privacy-Preserving Personalized Recommender Systems with Federated Learning
         Enmao: Privacy-preserving Personalized Recommender Systems with Partially Federated AutoEncoders
         Jie: Federated Recommender Systems with Private, Personalized, Partially Distributed AutoEncoders
         ...
         Enmao Diao enmao.diao@duke.edu
         Vahid Tarokh vahid.tarokh@duke.edu
         ...
         Recommender Systems (RCs) have become increasingly important in many domains such as digital marketing, personalized medicine, and data mining. Conventional RCs often need to collect users' data, centralize them on the server-side, and form a global model to generate reliable recommendations. However, they suffer from two critical limitations: the personalization problem that the RCs learned traditionally may not be customized for individual users, and the privacy problem that directly sharing user data is not encouraged or even prohibited. We propose Personalized Federated Recommender Systems (PersonalFR), which introduces a personalized autoencoder-based recommendation model with Federated Learning (FL) to address these challenges. PersonalFR guarantees that each user can learn a personal model from the local dataset and other participating users' data without sharing local data, data embeddings, or models. PersonalFR consists of three main components, including AutoEncoder-based RCs (ARCs) that learn the user-item interactions, Partially Federated Learning (PFL) that updates the encoder locally and aggregates the decoder on the server-side, and Partial Compression (PC) that only computes and transmits valuable model parameters. Extensive experiments on two real-world datasets demonstrate that PersonalFR can achieve private and personalized performance comparable to that trained by centralizing all the users' data. Moreover, PersonalFR needs significantly less computation and communication overhead than standard FL baselines.
      
      10. 摘抄
      
         1. Collaborative Filtering (CF) [9] is one of the most commonly used recommendation methods. CF consists in predicting whether, or how much, a user will like (or dislike) an item by leveraging the knowledge of the user preferences, as well as that of other users.
      
         2. It is obvious that a centralized recommendation algorithm induces a massive data transfer, which will cause a bottleneck problem in the data center. This clearly shows the importance of developing a distributed recommendation solution.
      
         3. In general, recommender systems are data-driven. The more data it uses, the better performance it obtains.
      
         4. experimental section可以提问题
      
         5. Federated learning is an emerging technology, which bridges the data silos and builds machine learning models without compromising user privacy and data security.
      
         6. However, because of the drawbacks of collaborative-based methods such as sparsity, cold start, etc., more attention has been drawn to hybrid methods that consider both the rating and content information.
      
         7. The model learns deep latent representations from content data in an unsupervised manner and also learns implicit relationships between items and users from both content and rating.
      
         8. With the rapid growth and high prevalence of Internet services and 
            applications, people have access to large amounts of online multi- 
            media content, such as movies, music, news and articles. While this 
            growth has allowed users to be able to consume a huge number of 
            resources with only one click, it has also made it more difficult for users to find information relevant to their interests. For example, 
            users might be not aware of the existance of interesting movies 
            they would like and researchers might nd it dicult to search for 
            important scientic articles related to their area of research. ere- 
            fore, recommender systems are becoming increasingly important 
            to aract users, and make eective use of the information available.
      
         9. Unlike CTR, with the representation power of deep neural network, the CVAE model learns more eective representations for content than topic models
      
         10. Unlike denoising autoencoders [26, 29], the CVAE model does not corrupt the input, **（可以用来说加了dp的ae不好，不止corrupt还有额外计算）**
      
         11. In this second use case, we believe that by exploiting and combining all these valuable data sources provided by different online services, we could make the recommender system more precise. The data sources are located and distributed over the clusters of different data centers, which are 
             geographically distributed. In this second use case, we assume that the recommendation system can identify and link entities that refer to the same users and items across the different data sources. We envision that the connection of these online services may be greatly helped by initiatives like OpenID (http://openid.net/), which promotes a uniﬁed user identity across different services. In addition, we assume that the online services are willing to help the recommender system through contracts that can be established.
      
         12. Furthermore, because of the stochasticity in latent space **（可以解释privacy）**
      
         13. ![image-20220828205831938](/Users/qile/Library/Application Support/typora-user-images/image-20220828205831938.png)图可以参考
      
         14. Another interpretation of the proposed CVAE model is from the 
             point of view of denoising. e inference network produces the 
             posterior variational distribution for the latent content variable z . 
             is can also be viewed as as corrupting the latent content vector 
             with Gaussian noise. Unlike the stacked denoising autoencoder 
             (SDAE) as proposed in [26, 29], where the input observations are 
             rst corrupted with various noise (masking noise, Gaussian noise, 
             salt-and-peper noise, etc), the corruption of data in latent space 
             avoids the diverse interpretation and corruption scheme of the ob- 
             servation space and provides a unied way of corruption. Also, the 
             noise level is automatically learned through the inference network, 
             instead of manually tuned like in SDAE. is leads to more robust 
             and systematic learning of representation regardless of the data 
             type it is trying to model.
      
         15. ![image-20220828212418028](/Users/qile/Library/Application Support/typora-user-images/image-20220828212418028.png)
      
         16. On the other hand, it is well known that the overwhelming performance of DNN- 
             based models is largely attributed to the increased access to large-scale training data. Such recommender systems are usually deployed on a cloud server to ac- 
             quire the suﬃcient resource for data storage and efﬁcient training, which then provides services to connected users in an on-demand manner. 
      
         17. Unfortunately, this centralized recommendation paradigm inevitably leads to in- 
             creasing concerns on user privacy as user data stored on the server might be accidentally leaked or misused. Even though the data sent to the server can be anonymized, users’   information   can   still   be   identiﬁed   when   linked 
             with  other  data  sources  [24] (Fung, B.C., Wang, K., Chen, R., Yu, P.S.: Privacy- 
             preserving data publishing: A survey of recent developments. ACM Computing Surveys 42(4), 1–53 (2010))
      
         18. Training   an   FL model   consists   of   several   key   steps.   First,   the   server 
             selects  a  batch  of  available  devices  and  sends  them  the 
             current global model. Then, based on locally stored user 
             data,  each  device  computes  the  gradients  w.r.t.  an  ob- 
             jective function of the model. Finally, the central system 
             aggregates  the  gradients  from  diﬀerent  devices  in  order 
             to   update   the   global   model.   These   training   steps   are 
             iterated   until   the   global   model   converges.   Each   user’s 
             data  is  fully  kept  on  her/his  personal  device  in  the  FL 
             setting   and   thus   they   retain   control   over   his/her   own 
             data. (描述fl)
      
         19. Generally, the more complex side information has been incorporated, the 
             higher quality recommendation can be achieved
      
         20. Deep learning models can learn high-order and non-linear latent 
             representations via various types of activation functions (e.g., 
             sigmoid, ReLu)
      
         21. Data Heterogeneity. It is common that users have 
             highly diﬀerent personal preferences when consuming 
             online products. Hence, the data heterogeneity prob- 
             lem arises in FL-based recommenders where the local 
             data generated by diﬀerent users vary signiﬁcantly in 
             both distribution and scale. This problem will be exac- 
             erbated for recommendation models due to two reasons. 
             First, frequent users have far more interaction records 
             with items than other inactive (e.g., new or cold-start) 
             users. Therefore, in a typical FL setting, straightfor- 
             wardly minimizing the average loss for all users will 
             mostly advantage the recommendation performance for 
             active users while the inactive users’ preferences can be
      
             hardly estimated. On this occasion, the ultimate recom-
      
             mendation model is clearly unfair for a huge number of 
             users due to biased service quality. Second, the diﬃ- 
             culty in achieving optimal personalization also arises 
             with data heterogeneity. Existing FL-based recommen- 
             dation paradigms are only designed to let all devices 
             contribute to a global model, hence the ﬁnal model is 
             only good on average recommendation accuracy. Since 
             they do not account for the heterogeneity of data dis- 
             tribution among users, there is no guarantee that the 
             global model is fully customized for every user, imped- 
             ing the delivery of personalized recommendation
      ...
      11. Paper reading:
          ...
          1. info:
             ...
             1. Exploit Side Information for Recommender System
      
          2. CF
      
             1. A Deep Framework for Cross-Domain and Cross-System Recommendations
                1. matrix factorization and DNN
                2. cross-domain recommendation / cross-system recommendation (解决data sparsity问题)
             2. A Distributed Collaborative Filtering Algorithm Using Multiple Data Sources （2018年）
             3. 分布式cf，mf,  **主要是加入了item的信息**
             4. Diverse internal data sources：It is obvious that a centralized recommendation algorithm induces a massive data transfer, which will cause a bottleneck problem in the data center. This clearly shows the importance of developing a distributed recommendation solution. （可以用来说为什么要分布式）
             5. Users’ preferences history:
                1. 用户喜好
             6. Users’ attributes:
                1. 用户信息
             7. Items’ attributes:
                1. 商品信息
             8. A  Uniﬁed  Framework  for  Cross-Domain  and Cross-System  Recommendations (2021年)
      
          3. AE：
      
             1. **Collaborative Variational Autoencoder for Recommender Systems （2017年，intro写得好）**
                1. **Bayesian + AE**
                2. learns a latent distribution for content in latent space
      
          4. FL:
      
             1. A Federated Recommender System for Online Learning Environments（2012年）
                1. presents a federated recommender system, which exploits data from different online learning platforms and delivers personalized recommendation (**主要是on people’s interaction with distributed resources)**
             2. A Federated Recommender System for Online Services (2020年)
                1. **对Federated  Recommender System 做了下各种算法的implementation**
             3. **Fast-adapting  and  Privacy-preserving  Federated  Recommender** 
                **System （2021年, 介绍了data heterogeneity, 可以借鉴）**
                1. Federated learning + meta learning + dp
                2. 
