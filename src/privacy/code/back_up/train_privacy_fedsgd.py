...
def train(data_loader, model, optimizer, metric, logger, epoch):
    ...
    # Iterate data_loader
    cur_data_point_num = 0
    model_name = cfg['model_name']
    
    for i, temp_input in enumerate(data_loader):
        for local_epoch in range(1, cfg[model_name]['local_epoch'] + 1):
            ...
            input = copy.deepcopy(temp_input)
            input = collate(input)
            ...
            if local_epoch % int(((cfg[model_name]['local_epoch'] + 1) * cfg['log_interval']) + 1) == 0:
