...
from utils import processed_folder, save, load, to_device, process_control, process_dataset, make_optimizer, make_scheduler, resume, collate, calculate_portion
...
def train(dataset, data_split, data_split_info, federation, metric, logger, epoch, global_optimizer_lr):
    ...
    num_average_nodes = int(np.ceil(cfg['partial_average_ratio'] * num_active_nodes))
   
    # print('num_active_nodes', num_active_nodes)
    local_parameters = {}
    start_time = time.time()
    model_name = cfg['model_name']
    if cfg['partial_average_shuffle'] == True:
        # print('dfafasf')
        old_node_average_idx = set()
        new_node_average_idx = set()
        for local_epoch in range(1, cfg[model_name]['local_epoch'] + 1):
            # if cfg['partial_average_shuffle'] == True:
            new_node_average_idx = set(random.sample(node_idx, num_average_nodes))
            partial_average_state_dict = federation.get_partial_average_state_dict()

            for m in range(num_active_nodes):
                # print('zz', local_epoch, m)
                # local_parameters[m] = copy.deepcopy(local[m].train(logger))
                if node_idx[m] in old_node_average_idx:
                    cur_model_train_state_dict = local[m].train(logger, federation, node_idx[m], 1, copy.deepcopy(partial_average_state_dict))
                else:
                    cur_model_train_state_dict = local[m].train(logger, federation, node_idx[m], 1)

                if node_idx[m] in new_node_average_idx:
                    federation.store_partial_average_state_dict(cur_model_train_state_dict, num_average_nodes)
                ...
                if local_epoch == cfg[model_name]['local_epoch'] and m % int((num_active_nodes * cfg['log_interval']) + 1) == 0:
            ...
            old_node_average_idx = copy.deepcopy(new_node_average_idx)
            new_node_average_idx = set()
    elif cfg['partial_average_shuffle'] == False:
        ...
        for local_epoch in range(1, cfg[model_name]['local_epoch']+1):
            ...
            partial_average_state_dict = federation.get_partial_average_state_dict()
            for m in range(len(rest_node_idx), len(node_idx)):
                # print('zz', local_epoch, m)
                # local_parameters[m] = copy.deepcopy(local[m].train(logger))
                cur_model_train_state_dict = local[m].train(logger, federation, node_idx[m], 1, copy.deepcopy(partial_average_state_dict))
                federation.store_partial_average_state_dict(cur_model_train_state_dict, num_average_nodes)
            ...
            if len(rest_node_idx) < len(node_idx):
                if local_epoch % int((cfg[model_name]['local_epoch'] * cfg['log_interval']) + 1) == 0:
                    local_time = (time.time() - start_time) / (m + 1)
                    epoch_finished_time = datetime.timedelta(seconds=local_time * (num_active_nodes - m - 1))
                    # exp_finished_time = epoch_finished_time + datetime.timedelta(
                    #     seconds=round((cfg['num_epochs']['global'] - epoch) * local_time * num_active_nodes))
                    exp_finished_time = 1
                    info = {'info': ['Model: {}'.format(cfg['model_tag']), 
                                    'Train Epoch: {}'.format(local_epoch),
                                    'ID: {}({}/{})'.format(node_idx[m], m + 1, num_active_nodes),
                                    'Epoch Finished Time: {}'.format(epoch_finished_time),
                                    'Experiment Finished Time: {}'.format(exp_finished_time)]}
                    logger.append(info, 'train', mean=False)
...
class Local:
    ...
    def train(self, logger, federation, cur_node_index, running_epoch, partial_average_state_dict=None):
        
        model = self.model
        model.to(cfg['device'])
        if partial_average_state_dict:
            model.load_state_dict(copy.deepcopy(partial_average_state_dict))
        model.train(True)
        ...
        for epoch in range(1, running_epoch+1):
            for i, original_input in enumerate(self.data_loader):
                ...
                # print('input_size', input_size, evaluation)
