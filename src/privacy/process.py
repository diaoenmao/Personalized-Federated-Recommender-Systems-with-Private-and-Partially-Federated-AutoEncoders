...
def draw_movielens_learning_curve():
    params_ele = cfg['model_tag'].split('_')
    new_params_ele = []
    for i in params_ele:
        new_params_ele.append([i])

    controls_list = [new_params_ele]
    controls = []
    ...
    # movielens_control_list = make_control_list('movielens')
    # cifar10_control_list = make_control_list('CIFAR10')
    # wikitext2_control_list = make_control_list('WikiText2')
    controls_list_1 = cifar10_control_list
    controls_1 = []
    for i in range(len(controls_list_1)):
        controls_1.extend(list(itertools.product(*controls_list_1[i])))
