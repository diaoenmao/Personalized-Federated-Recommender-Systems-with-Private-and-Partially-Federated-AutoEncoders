...
def runExperiment():
    ...
    for epoch in range(last_epoch, cfg[cfg['model_name']]['num_epochs'] + 1):
        ...
        test(dataset['test'], data_split['test'], data_split_info, federation, metric, logger, epoch)
        logger.safe(False)
...
def train(dataset, data_split, data_split_info, federation, metric, logger, epoch):
    ...
    federation.update_local_model_dict()
...
def test(dataset, data_split, data_split_info, federation, metric, logger, epoch):
    ...
    with torch.no_grad():
        ...
        for m in range(len(data_split)):
            ...
            model = federation.load_local_model_dict(m)['model']
            model.train(False)
