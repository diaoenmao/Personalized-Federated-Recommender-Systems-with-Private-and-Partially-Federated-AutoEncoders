...
class Federation:
    ...
    def new_global_model(self):
        self.global_encoder_model = copy.deepcopy(cfg['Encoder_instance'])
        self.reset_parameters(self.global_encoder_model)
        self.global_decoder_model = copy.deepcopy(cfg['Decoder_instance'])
            ...
            if cfg['train_mode'] == 'private':
                for key, value in self.global_decoder_model.named_parameters():
                    for m in range(len(local_parameters)):
                        a = self.global_decoder_model.state_dict()
                        b = local_parameters[m]
